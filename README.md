...
- **标准控件/布局重构（阶段4）** ✅ — app 层剩余手写渲染收敛为标准控件/布局表达：TopHeader 渐变标题手写 `TEXT styled`（`_title_runs`）→ **Gradient 标准控件**（`use_memo(deps=())` 缓存元素引用；Gradient 内部 `use_memo` 缓存渐变 runs 引用 → TEXT `_paint_cache` 跨帧命中，每帧零重建）；`_ParseLine` 手写 `~` 前缀替换 + 单 TEXT → **Row + InlineSpinner + TEXT** 标准控件/布局表达（与 _StreamingLine 同模式，帧序列与 `_fx.SPINNER_FRAMES` 同源，BUG-40 工具名内 `~` 不误替换保持）；**边框字符单一真源**：新增 `helpers.BORDER_CHARS`（single/double/round/bold/classic/dashed/singleDouble/doubleSingle），components（BOX 边框）/codeblock（CodeBlock）/model（工具卡边框）统一引用，删除 display.Panel 死常量 `_PANEL_BORDER_CHARS` 与 codeblock 本地副本；固化测试 `tests/test_tui/ink/test_standard_widgets_refactor.py`（11 例：Gradient 控件使用、_ParseLine 标准布局、边框字符单一真源、工具卡输出不变、App 树渲染不回归）
- **标准控件/布局重构（阶段5）** ✅ — **StatusBar 状态行**手写单 TEXT styled 组装 → **Row + 分段 TEXT** 标准控件/布局表达（前缀 2 列 / 模型名段 / 段间 2 空格 / 统计段独立元素，`_build_status_runs` 返回 `(model_runs, stats_runs)` 分段；超宽防御路径回退单 TEXT truncate_line——行宽不变量）；**工具卡片元素树化**：新增 `ToolCard` 标准控件组件（`src/tui/app/tool_card.py`，Column + TEXT 标准控件/布局表达，行构建复用 `_tool_card_styled_lines` 单一真源），ChatView 中工具卡（kind=="tool"）统一经 `h(ToolCard, ...)` 渲染——**工具卡行不再写入 committed_lines**（`commit_block`/`commit_open_block` tool 分支跳过、`close_tool_box` 移除 `_replace_committed_line` 图标翻转路径、`reflow_committed` 跳过），顶边框状态图标由 ToolCard 每帧从 `block.extra.tool_status` 读取（关闭后自动 ●→✔/✖）；**committed 区间发射**：content/reasoning 已提交块经 committed-chat 发射其在 `committed_lines` 中的连续区间 `[start, end)`（含卡片尾空行 trailer），committed-chat host 支持 start/end prop + 前缀缓存键含区间；**多 committed-chat 前缀复用**：`render_frame` 收集全部 committed-chat 前缀填入画布（单 committed-chat 保留稳定前缀 O(live) diff 路径，多 committed-chat 逐行 is 比较 O(总行数) 极快）；兼容路径（无 blocks 的渲染器级测试模型）整表发射。固化测试 `tests/test_tui/ink/test_standard_widgets_refactor.py` 阶段5（ToolCard 组件使用/工具卡不写 committed_lines/多 content 块区间发射渲染/关闭图标 ✔）+ `tests/test_tui/test_status_bar.py`（分段输出等价/防御截断）
