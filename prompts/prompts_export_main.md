...
| 场景 | prompt |
|------|--------|
| 全量探底 | `对当前项目执行完整探底（L1-L4），输出：所有核心模块的控制流图（强制——从 main 函数起全量追溯，合并为唯一一张控制流图）+ 数据流图 + 关联文件列表 + 重要文件列表（N. src/... 编号格式）` |
| 单模块 | `分析 src/<模块>/：控制流图（强制——从 main 函数起全量追溯，合并为唯一一张控制流图）+ 数据流图 + 关联文件列表 + 重要文件列表（N. src/... 编号格式）` |
| 修 Bug | `对 <函数名/模块名> 执行修bug前全量分析，产出：控制流图（强制——从 main 函数起全量追溯，合并为唯一一张控制流图）+ 数据流图 + 影响范围清单 + 关联文件列表 + 重要文件列表（N. src/... 编号格式）` |
...
## 变更总结
**修改范围**: <文件列表>
**控制流图**（强制——从 main 函数起全量追溯，合并为唯一一张控制流图）: <入口锚点→目标→叶子 完整调用路径；调用方/被调用方/调用方式/调用链路>
...
## 控制流图（强制——从 main 函数起全量追溯，合并为唯一一张控制流图）
