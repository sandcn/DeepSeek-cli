...
4. **确认无误后 → 直接进入执行阶段**：读完计划文件后禁止其他操作，直接根据计划文件的依赖与顺序信息并行派发 execute Agent 执行
...
### 读取并确认计划
通过 read_file 读取计划文件，从「步骤拆解」章节提取步骤列表、从「依赖与顺序」章节提取依赖关系，按拓扑分层推导可并行步骤组，校验完整性。**读完计划文件后禁止其他操作，直接根据计划文件的依赖与顺序信息并行派发 execute Agent 执行。** 若 plan Agent 失败且已降级为手动规划，则 `read_file` 读取手动生成的计划文件。
