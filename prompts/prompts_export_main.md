...
| 类图 | `分析 <类名>：类图（含完整继承链追溯至最顶层基类 + 管理类发现）/引用关系矩阵 + CFG + DFG + 关联文件列表（N. src/... 编号格式）` |
| 修 Bug | `对 <函数名/模块名> 执行修bug前全量分析，产出：反向全量追踪 + 正向≥2层 + CFG + DFG + 模块结构 + 类图/引用关系矩阵 + 隐式依赖 + 并发流程（如涉及）+ 影响范围清单 + 关联文件列表（N. src/... 编号格式）` |

> **prompt 必须显式要求输出全部维度**：CFG / DFG / 调用链 / 模块结构 / 类图（含继承链+管理类）/隐式依赖 / 关联文件列表。缺少任一 → 视为无效，须重新派发。**对非代码项目文件**（`.md` `.json` `.yaml` `.toml` `.ini` `.cfg` `.env` `.rst` `.txt`），map 的输出维度精简为：关联文件列表 + 内容结构摘要 + 被引用关系（豁免 CFG/DFG/类图）。**已发现 Bug 列表和可复用组件清单由 map Agent 内部铁律强制输出，无需在 prompt 中显式要求**。

### 执行之后干嘛
1. **验证六项铁律**：Agent 返回必须包含 CFG + DFG + 调用链 + 模块结构 + 类图（含继承链+管理类）/隐式依赖 + 关联文件列表。缺任一 → 视为无效，重新派发（≤2 次，超限则标注缺失后继续）。**对非代码项目文件**（`.md` `.json` `.yaml` `.toml` `.ini` `.cfg` `.env` `.rst` `.txt`），验证维度精简为关联文件列表 + 内容结构摘要 + 被引用关系，豁免 CFG/DFG/类图。**已发现 Bug 列表和可复用组件清单为强制输出但空列表不阻断验证**。
...
**派发前 map 前置（强制）**：派发 execute 前，必须确保对应修改范围的 map 全量信息（调用链/CFG/DFG/关联文件列表）已通过 map Agent 获取且有效，不得直接基于旧 map 结果或猜测派发。
...
## 类图
**类**: <名>  **继承链**: <当前→...→基类>  **管理类**: <列表>  **被引用**: <位置>  **依赖**: →<类>
