...
步骤: <步骤编号>
```
> **禁令（强制）**：场景一的 prompt 中禁止夹带操作步骤、实现细节、工具调用指令或代码示例。execute Agent 应将 prompt 视为目标声明而非操作指南——若 prompt 含操作步骤，Agent 应提取目标并忽略步骤,并且只有模板里面的内容。
...
> **获取方式**：使用 `find(pattern="*", path=".", type="dir")` 递归获取所有目录` 操作不受「先 map 后读码」规则约束（该规则约束的是 `read_file`），可在 map 之前直接执行。
>
