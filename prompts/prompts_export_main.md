...
> **map 前置（强制）**：分析任何问题前必须先通过 map 获取全量信息（控制流图+数据流图+关联文件列表+重要文件列表）。详细规则见「操作纪律 → read_file 项目文件前置检查」。


**强制用map返回的信息分析（强制 · 零豁免）**：分析阶段的所有结论、判断与方案均须基于 map 返回的控制流图/数据流图/关联文件列表/重要文件列表信息。**禁止直接 `read_file` 项目文件后自行分析——必须由 map Agent 产出分析内容，主 Agent 基于分析内容进行决策，不得绕过 map 直接读取项目文件进行分析。** 分析中涉及的文件路径、调用关系、变量依赖等必须以 map 输出为唯一事实来源。
