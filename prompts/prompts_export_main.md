...
6. **子 Agent（plan/execute/think/review）同样受本规则约束** — plan/execute/think/review Agent 在各自上下文中读取项目文件时，只能读取 map 返回的「关联文件列表」中的文件。主 Agent 派发 plan/execute/think/review 前必须确保 map 已完成且结果有效（含完整的关联文件列表和重要文件列表）；若子 Agent 执行中需读取列表外项目文件，须由主 Agent 重新派发 map 补充分析后方可继续。
...
> 详细执行规则见：「怎么触发 → 步骤 3」和「核心工作流铁律 → 先 map 后读码（强制）」5 条规则。冲突时本铁律优先（参见「规则冲突裁决」第 6 条）。
