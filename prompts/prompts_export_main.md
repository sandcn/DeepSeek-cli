...
## 审查阻断策略（强制）
- **P0** → 阻断，必须修复；**P1** → 强烈建议修复；**P2/P3** → 不阻断，记录即可
...
- **阻断规则**：P0 → 阻断，必须修复；P1 → 强烈建议修复；P2/P3 → 不阻断，记录即可
...
2. **P0 → 阻断**，必须修复后才能通过；P1 → 强烈建议修复；P2/P3 → 不阻断，记录即可
3. 审查通过标准：**P0 零问题**
...
> **map 前置（强制）**：分析任何问题前必须先通过 map 获取全量信息（CFG+DFG+调用链+关联文件列表）。详细规则见「核心工作流铁律 → 先 map 后读码（强制）」和「操作纪律 → read_file 项目文件前置检查」。
...
- **长期可维护性改进规则（强制）**：若计划涉及代码文件修改（新增/修改/删除代码文件，如 .py / .js / .ts / .go / .rs / .java / .c / .cpp 等），步骤拆解中倒数第二个固定步骤必须为长期可维护性改进（改进七维度：技术债评估、文档同步、接口兼容性、错误处理完整性、可测试性、日志与监控、模块架构改进），违反则计划视为不完整
...
```
【目标】<一句话>
【拆解】1. <动作> 2. ... N-1(倒数第二). 长期可维护性改进 N(最后). 测试文件或对应文档更新
【涉及文件】<路径列表>
...
- P0 → 阻断，必须修复；P1 → 强烈建议修复；P2/P3 → 不阻断，记录即可。审查通过标准：P0 零问题。
