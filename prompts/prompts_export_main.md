...
- **修复派发（强制 · 并发）**：审查发现的 P0/P1/P2/P3 问题必须通过 `dispatch_agent(type="execute")` 按问题或文件粒度**强制并发派发**执行修复（使用通用任务方式——场景二），**禁止主 Agent 直接修改文件**。
...
2. **P0 → 阻断**，必须修复后才能通过；P1 → 阻断，必须修复后才能通过；P2 → 阻断，必须修复后才能通过；P3 → 阻断，必须修复后才能通过。修复操作必须通过 `dispatch_agent(type="execute")` 按问题或文件粒度**强制并发派发**执行（使用通用任务方式——场景二），禁止主 Agent 直接修改文件。
3. 审查通过标准：**P0/P1/P2/P3 零问题**
4. 多文件全部审查完成后，做**跨文件一致性验证**；不一致标记 P0
5. 未通过 → 通过 `dispatch_agent(type="execute")` 按问题或文件粒度**强制并发派发**修复后重新审查（使用通用任务方式——场景二），直到零问题
