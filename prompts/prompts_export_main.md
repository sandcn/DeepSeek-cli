...
> **prompt 必须显式要求输出全部维度**：CFG / DFG / 调用链 / 模块结构 / 类图/隐式依赖 / 关联文件列表。缺少任一 → 视为无效，须重新派发。**对非代码项目文件**（`.md` `.json` `.yaml` `.toml` `.ini` `.cfg` `.env` `.rst` `.txt`），map 的输出维度精简为：关联文件列表 + 内容结构摘要 + 被引用关系（豁免 CFG/DFG/类图）。**已发现 Bug 列表由 map Agent 内部铁律强制输出，无需在 prompt 中显式要求**。

### 执行之后干嘛
1. **验证六项铁律**：Agent 返回必须包含 CFG + DFG + 调用链 + 模块结构 + 类图/隐式依赖 + 关联文件列表。缺任一 → 视为无效，重新派发（≤2 次，超限则标注缺失后继续）。**对非代码项目文件**（`.md` `.json` `.yaml` `.toml` `.ini` `.cfg` `.env` `.rst` `.txt`），验证维度精简为关联文件列表 + 内容结构摘要 + 被引用关系，豁免 CFG/DFG/类图。**已发现 Bug 列表为强制输出但空列表不阻断验证**。
...
**强制先用 map 得到全量信息后才分析问题（强制）**：map 必须完成且含 CFG + DFG + 调用链 + 模块结构 + 类图/隐式依赖 + 关联文件列表（与 map Agent 全量输出铁律六项对齐），缺少任一 → 重新派发（≤2 次）。map 返回结果缺少以上任一维度 → 视为分析不完整，禁止进入规划阶段，必须重新派发 map。对非代码项目文件（`.md` `.json` `.yaml` `.toml` `.ini` `.cfg` `.env` `.rst` `.txt`），按精简维度验证（关联文件列表 + 内容结构摘要 + 被引用关系）。
