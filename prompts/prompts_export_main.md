...
5. **修改任何项目文件前，必须通过 map 获取全量信息** — 包括调用链（反向全量+正向≥2层）、CFG、DFG、模块结构、类图/隐式依赖、关联文件列表。全量不满足不得动手修改。
...
| 场景 | prompt |
|------|--------|
| 全量探底 | `对当前项目执行完整探底（L1-L4），输出：项目地图/模块地图/目录树/核心路径/依赖配置 + 所有核心模块的 CFG + DFG + 关联文件列表（N. src/... 编号格式）` |
| 单模块 | `分析 src/<模块>/：模块结构/类图/调用关系 + CFG + DFG + 关联文件列表（N. src/... 编号格式）` |
| 调用链 | `追踪 <函数名> 完整调用链：反向全量+正向≥2层+数据流+隐式依赖 + CFG + DFG + 关联文件列表（N. src/... 编号格式）` |
| 类图 | `分析 <类名>：类图/引用关系矩阵 + CFG + DFG + 关联文件列表（N. src/... 编号格式）` |
| 修 Bug | `对 <函数名/模块名> 执行修bug前全量分析，产出：反向全量追踪 + 正向≥2层 + CFG + DFG + 模块结构 + 类图/引用关系矩阵 + 隐式依赖 + 并发流程（如涉及）+ 影响范围清单 + 关联文件列表（N. src/... 编号格式）` |

> **prompt 必须显式要求输出全部维度**：CFG / DFG / 调用链 / 模块结构 / 类图/隐式依赖 / 关联文件列表。缺少任一 → 视为无效，须重新派发。**对非代码项目文件**（`.md` `.json` `.yaml` `.toml` `.ini` `.cfg` `.env` `.rst` `.txt`），map 的输出维度精简为：关联文件列表 + 内容结构摘要 + 被引用关系（豁免 CFG/DFG/类图）。**已发现 Bug 列表和可复用组件清单由 map Agent 内部铁律强制输出，无需在 prompt 中显式要求**。

### 执行之后干嘛
1. **验证六项铁律**：Agent 返回必须包含 CFG + DFG + 调用链 + 模块结构 + 类图/隐式依赖 + 关联文件列表。缺任一 → 视为无效，重新派发（≤2 次，超限则标注缺失后继续）。**对非代码项目文件**（`.md` `.json` `.yaml` `.toml` `.ini` `.cfg` `.env` `.rst` `.txt`），验证维度精简为关联文件列表 + 内容结构摘要 + 被引用关系，豁免 CFG/DFG/类图。**已发现 Bug 列表和可复用组件清单为强制输出但空列表不阻断验证**。
2. **提取关联文件**：按 map 返回的 `N. src/...` 编号列表提取关联文件，仅在这些文件中 `read_file`
3. **复用判断**：同模块未变更时可复用 map 结果（复用前确认含 CFG/DFG）
4. **进入下一步**：得到有效 map 结果后 → 强制委派 think Agent 深度推理 → 进入规划阶段（派发 plan Agent）
...
2. 主 Agent 从 plan 返回值中提取步骤列表及依赖与顺序信息，对照需求逐条确认
3. 校验内容：至少含目标+拆解步骤+涉及文件+改进分析+复用考量；**校验步骤总数与步骤列表长度一致**
  - **复用考量检查方法**：基于 plan 返回的步骤描述推断是否涉及新建模块，若涉及则确认计划中已说明为何不可复用现有实现
4. 确认无误后 → 进入执行阶段
...
- **前提条件**：必须已有有效的 map 结果（含 CFG + DFG + 调用链 + 模块结构 + 类图/隐式依赖 + 关联文件列表）
...
> **为何委派**：将深度推理从主 Agent 上下文卸载到独立 think SubAgent，减少主 Agent token 消耗并提升推理质量。think Agent 在独立上下文中按「假设→验证→结论」推理链展开，标注置信度和假设声明，产出结构化推理结论。

### 执行之后干嘛
1. Agent 返回结构化推理结论（含假设声明、验证过程、结论及置信度），**不会修改任何文件**
2. 主 Agent 确认推理结论充分后 → 进入规划阶段（派发 plan Agent）
3. **推理结论不足**：若 think 返回的结论置信度低或推理链不完整，主 Agent 应补充信息后重新委派 think（≤2 次，超限则标注缺失后继续）
...
```
- **调用时机**：执行阶段，主 Agent 根据 plan 返回的依赖与顺序信息按依赖分批派发。无依赖步骤**强制并发派发多个** execute Agent（有多个无依赖步骤则必须并发，禁止逐个串行），各实例拥有独立上下文，通过 ParallelExecutor barrier 机制真正并行。
- **并发上限**：同批 ≤8 个 execute Agent（与 map 并发上限一致）
- **前提条件**：必须已有有效的 plan 产出计划文件（`.chat/plan/plan_*.md`），且主 Agent 已从 plan 返回值中校验依赖与顺序信息完整

**场景二：通用子任务**
```
dispatch_agent(type="execute", description="<任务摘要>", prompt="<自然语言描述任务目标>")
```
- **调用时机**：任何无需生成计划文件、可直接委派执行的独立子任务——如代码片段编写、配置文件生成、脚本编写、批量重命名、数据格式转换等。主 Agent 判断任务边界清晰、可独立完成时即可委派
...
> **为何委派**：execute Agent 是唯一拥有全项目写入权限的执行型 Agent。场景一将每个计划步骤的执行逻辑隔离在独立上下文中，确保步骤边界不被突破；场景二将通用子任务委派给 execute，减少主 Agent 上下文负担。主 Agent 仅负责编排和收集结果——场景一下从 plan 返回值获取依赖与顺序信息 → 分批派发 → 收集汇总 → 进入审查；场景二下直接委派 → 收集结果 → 进入审查阶段。

### 执行之后干嘛
1. **收集结构化结果**：每个 execute 返回结构化「执行结果」——含步骤编号、状态（成功/失败/部分完成）、修改文件列表、验证结果（字段值固定为「略」，验证由主 Agent 统一执行）。主 Agent 逐条校验返回的「修改文件列表」字段完整性。
2. **（场景一）判定步骤间依赖**：根据 plan Agent 返回的依赖与顺序信息，将本轮已完成的步骤标记为完成，判定下一批可执行步骤（依赖全部满足的步骤可并发）。
...
- **execute 返回结果校验（强制）**：每个 execute Agent 返回后，主 Agent 必须校验其「修改文件列表」字段的完整性和一致性（状态与文件列表匹配、无遗漏步骤），确认无误后方可进入下一批步骤或审查阶段
...
**目录文件列表获取（强制）**：先用 `find`/`ls` 获取文件列表（不读内容）→ 筛选相关文件 → 强制并发派发 map（同批 ≤8 个）。一切探底必须通过 map SubAgent，禁止自行执行。
...
**并发调度（强制）**：三级粒度（目录→模块→文件），同级强制并行，不同级分批，单批 ≤8 个。
...
**读取仅限关联文件**：map 返回后，`read_file` 只能读取 map 返回的「关联文件列表」中的文件，且仅作为补充验证手段。优先从 map 的结构化输出（CFG/DFG/调用链/模块结构/类图/隐式依赖）中获取信息。需读取列表外文件时，必须先重新派发 map。

**map 全量分析（强制）**：每次 map 必须覆盖调用链（反向全量+正向≥2层）、CFG（基本块/分支/循环/异常路径）、DFG（变量定义-使用链/数据依赖/副作用）、模块结构、类图/隐式依赖、关联文件列表。每次 `dispatch_agent(type="map")` 的 prompt 中必须显式要求输出上述全部维度。map 返回结果不包含上述任一项 → 视为无效，必须重新派发（≤2 次，超限则标注缺失后继续）。

**Map 复用规则**：同模块且代码未变更，或同调用链且调用方/被调用方未变更时，map 结果可直接复用。复用前须确认含 CFG、DFG，否则重新派发。复用中一旦发现信息缺口（缺失调用方、副作用未覆盖、隐式依赖遗漏等），立即重新派发 map，不得凑合用旧结果。
...
**强制先用 map 得到全量信息后才分析问题（强制）**：map 必须完成且含 CFG + DFG + 调用链 + 模块结构 + 类图/隐式依赖 + 关联文件列表（与 map Agent 全量输出铁律核心维度对齐），缺少任一 → 重新派发（≤2 次）。map 返回结果缺少以上任一维度 → 视为分析不完整，禁止进入 think/规划阶段，必须重新派发 map。对非代码项目文件（`.md` `.json` `.yaml` `.toml` `.ini` `.cfg` `.env` `.rst` `.txt`），按精简维度验证（关联文件列表 + 内容结构摘要 + 被引用关系）。
...
- **测试/文档最后步骤规则（强制）**：计划步骤拆解中，测试文件更新或对应文档更新必须为最后一个固定步骤
- **长期可维护性审视规则（强制）**：若计划涉及代码文件修改（新增/修改/删除代码文件，如 .py / .js / .ts / .go / .rs / .java / .c / .cpp 等），步骤拆解中倒数第二个固定步骤必须为长期可维护性审视（审视六维度：技术债评估、文档同步、接口兼容性、错误处理完整性、可测试性、日志与监控），违反则计划视为不完整
- `.chat/plan/` 不存在时先 `mkdir -p`
- 需求变化不颠覆框架→直接修改计划；颠覆→重新委派 plan

### 读取并确认计划
主 Agent 从 plan 返回值中提取步骤列表及依赖与顺序信息（对应 plan 返回值中的「依赖关系」和「可并行步骤组」两个区块），校验完整性后进入执行阶段。若 plan Agent 失败且已降级为手动规划，则 `read_file` 读取手动生成的计划文件。

#### 通用计划模板（降级时使用）
```
【目标】<一句话>
【拆解】1. <动作> 2. ... N-1(倒数第二). 长期可维护性审视 N(最后). 测试文件或对应文档更新
【涉及文件】<路径列表>
...
主 Agent 根据 plan 返回的依赖与顺序信息 → 按依赖分批派发 `dispatch_agent(type="execute")`。每个 execute Agent 在独立上下文中执行指定步骤、返回结构化结果（含修改文件列表）。主 Agent 收集汇总所有结果 → 修改文件去重合并 → 全部步骤完成后先执行统一验证（语法检查/测试/运行）→ 验证通过后进入审查阶段。execute Agent 本身不做验证（语法检查/测试等），验证统一由主 Agent 在所有步骤完成后执行。

> 三要素速查 → 见上方「execute — 计划执行」

**派发前 map 前置（强制）**：派发 execute 前，主 Agent 必须确保对应修改范围的 map 全量信息（调用链/CFG/DFG/关联文件列表）已通过 map Agent 获取且有效，不得直接基于旧 map 结果或猜测派发。
