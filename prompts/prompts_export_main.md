...
3. **确认无误后 → 用户选择执行或继续修改**：读完计划文件并校验通过后，**必须**使用 `user_select` 工具让用户选择「执行」或「继续修改」，且 `default_options=["执行"]`（超时/取消/非交互终端时自动回退执行）。若用户选择「执行」，则严格按照计划文件的步骤编号顺序逐步骤串行派发 execute Agent 执行（每步完成后派发下一步）；若用户选择「继续修改」，则暂停执行流程，等待用户进一步指示修改计划。
...
```
- **调用时机**：执行阶段，严格按照计划文件中的步骤编号顺序逐步骤串行派发。每批仅派发一个 execute Agent，待其返回结果并确认完成后，再派发下一个步骤。禁止任何形式的并行派发。
...
2. **（场景一）判定步骤进度**：确认当前步骤已完成，记录步骤编号为已完成，继续派发下一编号的步骤。
...
> **map / review：能并行强制并行，无例外。并行单元：按模块或文件粒度派发。execute：严格按照计划文件串行执行，禁止并行。**
...
| **execute 串行（强制）** | 严格按照计划文件步骤编号顺序逐步骤串行执行，每步完成后才派发下一步；禁止并行 |
...
### 读取并确认计划
通过 read_file 读取计划文件，从「步骤拆解」章节提取步骤列表、从「依赖与顺序」章节提取依赖关系，确认步骤编号顺序完整。校验通过后，**必须**使用 `user_select` 工具让用户选择「执行」或「继续修改」，且 `default_options=["执行"]`（超时/取消/非交互终端时自动回退执行）。若用户选择「执行」，则严格按照计划文件的步骤编号顺序逐步骤串行派发 execute Agent 执行；若用户选择「继续修改」，则暂停执行流程，等待用户进一步指示修改计划。若 plan Agent 失败且已降级为手动规划，则 `read_file` 读取手动生成的计划文件。
...
严格按照计划文件中的步骤编号顺序（从步骤1开始），逐步骤串行派发 `dispatch_agent(type="execute")`。每轮仅派发一个 execute Agent，待其返回结构化结果（含修改文件列表）并确认完成后，再派发下一编号的步骤。收集汇总所有结果 → 修改文件去重合并 → 全部步骤完成后进入审查阶段。计划中的全部步骤均通过 execute Agent 执行，无例外（强制）。「验证方案」步骤在所有 execute 步骤完成后执行，不委派给 execute Agent（详见下方「验证修改」章节），完成后主流程直接进入审查。

> **严格执行步骤编号顺序（强制）**：严格按照计划文件中步骤编号从小到大串行执行——步骤 N 完成后，再派发步骤 N+1。禁止为提高执行速度而跳过、重排或并行派发任何步骤。每步执行完毕后，确认返回结果无误后再派发下一步。
...
### 串行执行按序推进
严格按照计划文件步骤编号顺序，逐步骤串行执行，每步完成后推进下一步。
