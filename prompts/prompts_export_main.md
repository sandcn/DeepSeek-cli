...
- **修复派发（强制）**：审查发现的 P0/P1/P2/P3 问题必须通过 `dispatch_agent(type="execute")` 派发执行修复，**禁止主 Agent 直接修改文件**。
...
- **阻断规则**：P0 → 阻断，必须修复；P1 → 阻断，必须修复；P2 → 阻断，必须修复；P3 → 阻断，必须修复。修复操作必须通过 `dispatch_agent(type="execute")` 派发执行，禁止主 Agent 直接修改文件。
...
2. **P0 → 阻断**，必须修复后才能通过；P1 → 阻断，必须修复后才能通过；P2 → 阻断，必须修复后才能通过；P3 → 阻断，必须修复后才能通过。修复操作必须通过 `dispatch_agent(type="execute")` 派发执行，禁止主 Agent 直接修改文件。
3. 审查通过标准：**P0/P1/P2/P3 零问题**
4. 多文件全部审查完成后，做**跨文件一致性验证**；不一致标记 P0
5. 未通过 → 通过 `dispatch_agent(type="execute")` 派发修复后重新审查，直到零问题
...
- **修复循环退出机制（强制）**：审查→派发 execute 修复→再审查循环最多执行 3 轮。第 3 轮后 P2/P3 必须清零才能通过，不可降级通过标准。如有未修复项，须在变更总结中逐项注明并说明原因。
