...
```
- **调用时机**：执行阶段，主 Agent 读取并校验计划文件后，按计划文件「依赖与顺序」章节逐批派发。无依赖步骤**强制并发派发多个** plan_execute Agent（有多个无依赖步骤则必须并发，禁止逐个串行），各实例拥有独立上下文，通过 ParallelExecutor barrier 机制真正并行。
...
### 执行之后干嘛
1. **收集结构化结果**：每个 plan_execute 返回结构化「执行结果」——含步骤编号、状态（成功/失败/部分完成）、修改文件列表、验证结果（字段值固定为「略」，验证由主 Agent 统一执行）。主 Agent 逐条校验返回的「修改文件列表」字段完整性。
...
5. **全部步骤完成后**：先执行统一验证（语法检查/测试/运行验证），验证通过后 → 进入审查阶段——派发 review Agent，审查须覆盖所有 plan_execute 累计修改的全部文件。
...
| plan_execute 并发 | 无依赖步骤强制并发派发多个 plan_execute，各实例独立上下文，同批 ≤8 个 |
...
主 Agent 读取计划文件 → 解析「依赖与顺序」→ 按依赖分批派发 `dispatch_agent(type="plan_execute")`。每个 plan_execute Agent 在独立上下文中执行指定步骤、返回结构化结果（含修改文件列表）。主 Agent 收集汇总所有结果 → 修改文件去重合并 → 全部步骤完成后先执行统一验证（语法检查/测试/运行）→ 验证通过后进入审查阶段。plan_execute Agent 本身不做验证（语法检查/pytest 等），验证统一由主 Agent 在所有步骤完成后执行。

> 三要素速查 → 见上方「plan_execute — 计划执行」

**派发前 map 前置（强制）**：派发 plan_execute 前，主 Agent 必须确保对应修改范围的 map 全量信息（调用链/CFG/DFG/关联文件列表）已通过 map Agent 获取且有效，不得直接基于旧 map 结果或猜测派发。
