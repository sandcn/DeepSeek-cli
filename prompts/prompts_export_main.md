...
| 场景 | prompt |
|------|--------|
| 全量探底 | `对当前项目执行完整探底（L1-L4），输出：所有核心模块的 CDAG（从主入口开始追溯）+ VDAG + 关联文件列表 + 重要文件列表（N. src/... 编号格式）` |
| 单模块 | `分析 src/<模块>/：CDAG（从主入口开始追溯）+ VDAG + 关联文件列表 + 重要文件列表（N. src/... 编号格式）` |
| 修 Bug | `对 <函数名/模块名> 执行修bug前全量分析，产出：CDAG（从主入口开始追溯）+ VDAG + 影响范围清单 + 关联文件列表 + 重要文件列表（N. src/... 编号格式）` |
...
**强制用map返回的信息分析（强制）**：分析阶段的所有结论、判断与方案均须基于 map 返回的 CDAG/VDAG/关联文件列表/重要文件列表信息，禁止绕过 map 信息直接基于猜测或主观判断进行分析。分析中涉及的文件路径、调用关系、变量依赖等必须以 map 输出为唯一事实来源。
...
## 变更总结
**修改范围**: <文件列表>
**CDAG**: <入口锚点→目标→叶子 完整调用路径；调用方/被调用方/调用方式/调用链路>
