...
5. **修改任何项目文件前，必须通过 plan Agent 产出完整计划** — 计划须含目标 + 拆解步骤 + 涉及文件 + 验证方案。计划不完整（缺任一项）不得动手修改。
...
3. **失败处理**：客观失败 ≥2 次暂停审视、调整方案；连续 3 次失败停止并报告。
