...
## 操作纪律
- **禁止直接读取项目文件（强制 · 一票否决）**：**禁止直接使用 `read_file`/`search`/`bash cat` 读取任何项目文件内容。所有项目文件信息必须通过 `dispatch_agent(type="map")` 获取分析结果（控制流图/数据流图/关联文件列表/重要文件列表）。用户明确指定读取的文件是唯一例外**（按下方决策树第一条分支处理）。以下理由不构成豁免：「文件很小」「只是确认一下」「只读一行」「快速看一眼」「零逻辑变更」。任何绕过 map 直接读取项目文件的行为均视为规则违规。
