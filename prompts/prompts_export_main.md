...
- **审查（强制 · 零豁免）**：只要修改了文件就**必须**经 review SubAgent 审查，零问题即通过。无论修改规模大小、是否「零逻辑变更」，绝无例外。
...
5. **修改任何项目文件前，必须通过 map 获取全量信息** — 包括调用链（反向全量+正向≥2层）、CFG、DFG、状态机图、模块结构、类图/引用关系矩阵、隐式依赖、关联文件列表。全量不满足不得动手修改。
...
```
- **调用时机**：只要有信息缺口就调用，不限于首次——工作流任何阶段，只要现有信息不足以做出可靠决策（含读/改任何源码前），立即派发 map。典型触发：首次接触项目、接手新模块、跨模块修改、Bug 追踪、执行中遇到未预期的依赖/调用关系/副作用
...
> **prompt 必须显式要求输出全部维度**：CFG / DFG / 状态机图 / 调用链 / 模块结构 / 类图/隐式依赖 / 关联文件列表。缺少任一 → 视为无效，须重新派发。

### 执行之后干嘛
1. **验证七项铁律**：Agent 返回必须包含 CFG + DFG + 状态机图 + 调用链 + 模块结构 + 类图/隐式依赖 + 关联文件列表。缺任一 → 视为无效，重新派发（≤2 次，超限则标注缺失后继续）
...
- **map 关联文件列表**：所有关联文件，`N. src/...` 格式逐行列出
- **前向兼容性要求（强制）**：需评估接口稳定性、数据模型可演进性、扩展阻塞风险、关键假设文档化
...
- map 关联文件列表（所有关联文件，`N. src/...` 格式）

### 执行之后干嘛
1. Agent 返回 P0/P1/P2/P3 分级审查结果
...
- 修改须与现有代码风格一致
- 文档/注释须同步更新
- **所有修改必须经 review SubAgent 审查（强制 · 零豁免）**：只要修改了文件就要强制 review，无例外
...
**触发场景**：只要有信息缺口就调用 map，不限于工作流初始阶段。典型场景包括但不限于：首次接触→全量；接手新模块→模块分析；跨模块/Bug→模块+调用链；方案评估→候选模块并行；读/改单文件→分析该模块；执行中遇未预期依赖/副作用/调用关系→立即重新派发 map。
...
**map 全量分析（强制）**：每次 map 必须覆盖调用链（反向全量+正向≥2层）、CFG（基本块/分支/循环/异常路径）、DFG（变量定义-使用链/数据依赖/副作用）、状态机图（状态/转换/守卫/动作）、模块结构、类图/引用关系矩阵、隐式依赖、关联文件列表。每次 `dispatch_agent(type="map")` 的 prompt 中必须显式要求输出上述全部维度。map 返回结果不包含上述任一项 → 视为无效，必须重新派发（≤2 次，超限则标注缺失后继续）。
...
### 先审查再完成（强制 · 零豁免）
只要修改了文件就**必须** `dispatch_agent(type="review")` 审查——无论改一行还是改一百行、无论源码还是文档，绝无例外。多文件并发派发。
