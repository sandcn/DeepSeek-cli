...
- **测试与代码同步规则（强制）**：计划步骤拆解中，每个修改代码的步骤必须在子步骤中包含对应的测试文件或文档更新，不再单独设立末步骤集中处理测试。代码集成与端到端串联为倒数第二个固定步骤，长期可维护性改进为最后一个固定步骤
- **长期可维护性改进规则（强制）**：若计划涉及代码文件修改（新增/修改/删除代码文件，如 .py / .js / .ts / .go / .rs / .java / .c / .cpp 等），步骤拆解中最后一个固定步骤必须为长期可维护性改进（改进八维度：技术债评估、文档同步、接口兼容性、错误处理完整性、可测试性、日志与监控、模块架构改进、过大函数/文件切分），违反则计划视为不完整
- **代码集成与端到端串联规则（强制）**：若计划涉及代码文件修改（新增/修改/删除代码文件，如 .py / .js / .ts / .go / .rs / .java / .c / .cpp 等），步骤拆解中倒数第二个固定步骤必须为代码集成与端到端串联，违反则计划视为不完整
- **环境/应用配置步骤规则（强制）**：若计划涉及环境配置或应用配置变更（环境变量/配置文件/配置项），步骤拆解中须在代码集成与端到端串联步骤之前（倒数第三）新增配置步骤（内容至少含：受影响配置盘点、配置模板同步、默认值与校验），违反则计划视为不完整
...
```
【目标】<一句话>
【拆解】1. <动作>+测试 2. <动作>+测试 ... [N-2(如有配置变更). 环境/应用配置更新] N-1(倒数第二). 代码集成与端到端串联 N(最后). 长期可维护性改进
【涉及文件】<路径列表>
