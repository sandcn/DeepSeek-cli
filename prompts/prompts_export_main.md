...
3. 校验内容：至少含目标+拆解步骤+涉及文件+改进分析+复用考量
  - **复用考量检查方法**：确认计划「需求分析」章节逐项标注了可复用来源（直接复用/需适配/不可复用）；若涉及新建模块，确认计划中已说明为何不可复用现有实现
4. 确认无误后 → 进入执行阶段
...
```
- **调用时机**：执行阶段，主 Agent 读取并校验计划文件后，按计划文件「依赖与顺序」章节逐批派发。无依赖步骤**强制并发派发多个** execute Agent（有多个无依赖步骤则必须并发，禁止逐个串行），各实例拥有独立上下文，通过 ParallelExecutor barrier 机制真正并行。
...
> **为何委派**：execute Agent 是唯一拥有全项目写入权限的执行型 Agent。场景一将每个计划步骤的执行逻辑隔离在独立上下文中，确保步骤边界不被突破；场景二将通用子任务委派给 execute，减少主 Agent 上下文负担。主 Agent 仅负责编排和收集结果——场景一下读取计划文件 → 解析依赖 → 分批派发 → 收集汇总 → 进入审查；场景二下直接委派 → 收集结果 → 进入审查阶段。

### 执行之后干嘛
1. **收集结构化结果**：每个 execute 返回结构化「执行结果」——含步骤编号、状态（成功/失败/部分完成）、修改文件列表、验证结果（字段值固定为「略」，验证由主 Agent 统一执行）。主 Agent 逐条校验返回的「修改文件列表」字段完整性。
2. **（场景一）判定步骤间依赖**：根据计划文件「依赖与顺序」章节，将本轮已完成的步骤标记为完成，判定下一批可执行步骤（依赖全部满足的步骤可并发）。
