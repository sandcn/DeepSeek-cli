...
> **map 前置（强制）**：分析任何问题前必须先通过 map 获取全量信息（CFG+DFG+关联文件列表+重要文件列表）。详细规则见「操作纪律 → read_file 项目文件前置检查」。
...
### 代码修改工作流
修改前必须通过 map 获取 CFG + DFG（不得为判定「是否零逻辑」而绕过 map，零逻辑变更不豁免 map）。逐处验证：map → 读计划 → 输出修改范围声明 → 精确 old_string 定位 → 一处一改 → 审查+验证。
...
**DFG**（强制）: <产生/消失/作用/更新 四维度 — 每维度标注行号>（第三方库路径下的文件豁免本要求，参见 read_file 前置检查决策树的系统/第三方库路径定义）
**改动内容**: <说明>  **改动动机**: <原因>
**未做事项**: <有意搁置>
