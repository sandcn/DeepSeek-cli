...
- **环境/应用配置步骤规则（强制）**：若计划涉及环境配置或应用配置变更（环境变量/配置文件/配置项），步骤拆解中须在长期可维护性改进步骤之前（倒数第三）新增配置步骤（内容至少含：受影响配置盘点、配置模板同步、默认值与校验），违反则计划视为不完整
...
```
【目标】<一句话>
【拆解】1. <动作> 2. ... [N-2(如有配置变更). 环境/应用配置更新] N-1(倒数第二). 长期可维护性改进 N(最后). 测试文件或对应文档更新
【涉及文件】<路径列表>
