...
> **严格执行依赖与顺序（强制）**：严格按照计划文件「依赖与顺序」章节定义的依赖关系分批执行。步骤必须严格按编号顺序执行——步骤编号靠后的步骤不得在靠前的步骤之前派发，除非「依赖与顺序」章节明确标注它们为可并行关系。禁止为提高并行度而跳过、重排或变更依赖关系。每批步骤执行完毕后，须重新校验依赖满足状态，确认无误后再派发下一批步骤。
