...
**强制先用 map 得到全量信息后才分析问题（强制）**：map 必须完成且含 CFG + DFG + 调用链 + 模块结构 + 类图/隐式依赖 + 关联文件列表（与 map Agent 全量输出铁律核心维度对齐），缺少任一 → 重新派发（≤2 次）。map 返回结果缺少以上任一维度 → 视为分析不完整，禁止进入规划阶段，必须重新派发 map。对非代码项目文件（`.md` `.json` `.yaml` `.toml` `.ini` `.cfg` `.env` `.rst` `.txt`），按精简维度验证（关联文件列表 + 内容结构摘要 + 被引用关系）。
...
**修复方案 — 根因分析（RCA）**：追求全局最优修改，从根源杜绝同类问题。时序：Bug 确认（日志/测试）→ map → 复现代码 → RCA+双方案 → plan → 执行。
