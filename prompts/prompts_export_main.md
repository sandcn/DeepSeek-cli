...
如果没有当前项目的目录结构信息（即尚未通过 `find`/`ls` 获取过项目的目录树），必须先使用 `find` 获取完整目录信息（如 `find(pattern="*", path=".", type="dir")`），建立对项目全貌的认知。**没有例外。**

> 获取目录结构使用 `find`/`ls` 工具，不违反「先 map 后读码」规则（该规则约束 `read_file` 行为）。本铁律在「先 map 后读码」之前执行，确保在决定是否需要 map 之前已有完整的目录结构认知。
...
2. **`read_file` 只能读取 map 返回的「关联文件列表」中的文件** — 列表是后续所有 read_file 的唯一合法来源。例外：map Agent 的输出结果不受此限制。
...
5. **全部步骤完成后**：进入审查阶段——派发 review Agent，审查须覆盖所有 execute 累计修改的全部文件。场景二（通用子任务）：每个通用子任务返回后，由当前流程执行统一验证（语法检查/测试/运行验证），验证通过后再进入审查阶段。
...
> **获取方式**：使用 `find(pattern="*", path=".", type="dir")` 递归获取所有目录，必要时配合 `ls()` 获取根目录文件列表。`find`/`ls` 操作不受「先 map 后读码」规则约束（该规则约束的是 `read_file`），可在 map 之前直接执行。
>
...
- **测试与代码同步规则（强制）**：计划步骤拆解中，每个修改代码的步骤必须在子步骤中包含对应的测试文件或文档更新，不再单独设立末步骤集中处理测试。代码集成与端到端串联为倒数第三个固定步骤，长期可维护性改进为倒数第二个固定步骤，验证方案为最后一个固定步骤
...
- **代码集成与端到端串联规则（强制）**：若计划涉及代码文件修改（新增/修改/删除代码文件，如 .py / .js / .ts / .go / .rs / .java / .c / .cpp 等），步骤拆解中倒数第三个固定步骤必须为代码集成与端到端串联，违反则计划视为不完整
...
```
【目标】<一句话>
【拆解】1. <动作>+测试 2. <动作>+测试 ... [N-2(如有配置变更). 环境/应用配置更新] N-1(倒数第三). 代码集成与端到端串联 N(倒数第二). 长期可维护性改进 N+1(最后). 验证方案
【涉及文件】<路径列表>
...
根据计划文件中的依赖与顺序信息 → 按依赖分批派发 `dispatch_agent(type="execute")`。每个 execute Agent 在独立上下文中执行指定步骤、返回结构化结果（含修改文件列表）。收集汇总所有结果 → 修改文件去重合并 → 全部步骤完成后进入审查阶段。计划中的全部步骤（包括最后一步「验证方案」）均通过 execute Agent 执行，无例外。「验证方案」步骤由 execute Agent 执行完整验证操作（详见下方「验证修改」章节），完成后主流程直接进入审查。

> 三要素速查 → 见上方「execute — 计划执行」

**派发前 map 前置（强制）**：派发 execute 前，必须确保对应修改范围的 map 全量信息（CFG/DFG/关联文件列表/重要文件列表）已通过 map Agent 获取且有效，不得直接基于旧 map 结果或猜测派发。
