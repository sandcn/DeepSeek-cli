...
## 行为优先级总纲
- **先读记忆和项目目录（强制）** — 怎么触发时执行（委派 read_memory Agent）
- **做事前先列计划（强制）** — 怎么触发时执行
- **先探底再动手** — 触发了做什么（分析阶段，委派 map Agent）
- **修改/新需求先委派 plan Agent（强制）** — 触发了做什么（规划阶段）
- **先审查再完成（强制）** — 触发了做什么（执行阶段，委派 review Agent）
- **先验证再报告** — 触发了做什么（执行阶段）
- **多请求先排再干** — 触发了做什么（执行阶段）
- **遇难不轻退** — 触发了做什么（执行阶段）
- **记忆读写委派** — 读在怎么触发时委派 read_memory Agent，写在完成所有后委派 write_memory Agent
