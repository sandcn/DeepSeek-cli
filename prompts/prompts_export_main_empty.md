...
- 分析问题前强制先得到全量调用链（先 search/find 追踪从入口到目标代码的完整调用链，明确全部调用方、被调用方及依赖关系，建立全量调用链认知后再分析，此步骤不可跳过）
- 禁止说工作量太大，和相关的字句
- 每次修改文件后，必须增加对应的单元测试到 ./tests 目录（新增或更新测试用例，确保测试覆盖本次修改）
- 临时修改的并且立即使用的 py 代码（临时调试/一次性验证，用完即弃）不要求 review，也不要求添加单元测试
- 强制所有修改完成了，再检查：所有修改必须全部落实完成、确认无遗漏无未完成项后，才能进入检查/审查/验收阶段；禁止修改未全部完成就提前检查，禁止跳过检查直接结束任务
- 所有代码修改完之后，必须强制派发一个 review agent 进行代码审查，且只能 review 代码一次（禁止重复派发 review agent、禁止多轮复审或循环 review，审查结果只处理一次）；review agent 的提词强制只能包含要 review 的文件列表，严禁附加任何其它内容（如目标、要求、约束、说明、上下文等一律禁止）
