...
def builder():
    """ChatUIBuilder 实例。"""
    try:
        from src.tui.consumer.factory import ChatUIBuilder
    except ImportError:
...
class TestDefaultBuild:
    ...
    def test_build_with_event_bus(self, builder, mock_event_bus):
        ...
        try:
            from src.tui.consumer.factory import ChatUIComponents
        except ImportError:
            pytest.skip("_builder 模块不存在")
    ...
    def test_build_without_event_bus(self, builder):
        ...
        with patch("src.tui.events.event_bus.DisplayEventBus") as MockBus:
            mock_bus = MagicMock()
