...
class TestBeauty26ToolIconBreath:
    ...
    def _tool_name_run(self, head):
        """找到工具名 run（加粗类别色，如 Bash；非状态图标/detail）。"""
        for run in head:
            if run.text and run.text.strip() == "Bash":
                return run
        return None

    def test_running_icon_uses_breath_style(self):
        """running 工具卡标题工具名呼吸色（bash→shell 类别色 41~49 脉动）。

        2026-08-06 用户需求（Claude Code 极简样式）：标题行去掉 emoji 工具
        图标/▎ 引导线——「● Bash echo hi」；工具名按类别着色——运行中在
        类别色邻域呼吸（bash 属 shell 类：41→49）。
        """
        from src.tui.app.toolcard import tool_card_lines
        blk = self._tool_block("running")
        lines = tool_card_lines(blk, 60)
        head = lines[0]
        # 结构：状态图标● + 工具名 Bash（类别呼吸，加粗）+ detail
        name_run = self._tool_name_run(head)
        assert name_run is not None, f"应找到工具名 run: {head!r}"
        # 运行中工具名色应为类别呼吸色（bash→shell 41~49）
        fg = name_run.style.fg if name_run.style else None
        assert fg is not None, "运行中工具名应有颜色"
        assert 41 <= fg <= 49, f"运行中工具名色应在类别呼吸区间(41~49): {fg}"

    def test_done_icon_static(self):
        """已关闭工具卡标题工具名静态类别色（bash→shell 41，frozen 缓存）。

        Claude Code 极简样式：关闭/提交后工具名静态类别色（标题行状态图标
        原位翻转 ✔，工具名色不变）。
        """
        from src.tui.app.toolcard import tool_card_lines
        ...
        assert name_run is not None, f"应找到工具名 run: {head!r}"
        fg = name_run.style.fg if name_run.style else None
        assert fg == 41, f"已关闭卡工具名应静态类别色 41（shell）: {fg}"
