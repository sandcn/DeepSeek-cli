...
class TestTerminalWidthCache:
    ...
    def mock_terminal(self):
        """Mock Blessed Terminal 返回固定宽度 100、高度 40。"""
        with mock.patch(
            "src.tui.terminal.terminal.get_terminal"
        ) as mock_term:
            mock_term.return_value.width = 100
            mock_term.return_value.height = 40
            yield mock_term

    def test_get_width_returns_cached_value(self, mock_terminal):
        """get_width() 返回缓存宽度值。"""
        from src.tui.terminal.terminal import TerminalWidthCache
    ...
    def test_cached_values_persist_within_ttl(self, mock_terminal):
        ...
        mock_terminal.return_value.width = 200
        mock_terminal.return_value.height = 80
        w2 = cache.get_width()
        h2 = cache.get_height()
        # 应返回缓存值而非新值
        assert w2 == 100
    ...
    def test_force_refresh_bypasses_ttl(self, mock_terminal):
        ...
        assert cache.get_height() == 40
        # 修改 mock 返回值
        mock_terminal.return_value.width = 120
        mock_terminal.return_value.height = 50
        # force_refresh 绕过 TTL
        cache.force_refresh()
