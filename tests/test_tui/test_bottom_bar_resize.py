...
class TestShrinkProtection(unittest.TestCase):
    ...
    def test_shrink_triggers_scroll_up(self):
        """终端缩小时应上滚内容保护上屏末行。"""
        self.bb._last_text = "some text"

        # 终端从 30 行缩到 29 行
        with patch("src.ui._bottom_bar.query_terminal_size",
                   return_value=(80, 29)), \
             patch.object(sys, '__stdout__', io.StringIO()) as mock_out:
            self.bb._check_resize()

        output = mock_out.getvalue()
        # 应输出 \033[r 和 \033[29;1H\n 上滚保护序列
        self.assertIn("\033[r", output,
                       "缩小保护应输出全屏滚动序列")
        self.assertIn("\033[29;1H\n", output,
                       "缩小保护应从第29行底部上滚")

    def test_shrink_scrolls_correct_delta_lines(self):
        """缩小 2 行时应上滚 2 行。"""
        self.bb._last_text = "some text"
        ...
        output = mock_out.getvalue()
        # \033[28;1H\n 应出现 2 次（缩小 2 行）
        count = output.count("\033[28;1H\n")
        self.assertEqual(count, 2,
                         f"缩小 2 行时应上滚 2 行，实际 {count}")

    def test_grow_does_not_trigger_scroll_up(self):
        """终端变高时不触发缩小保护。"""
        self.bb._last_text = "some text"
        ...
        output = mock_out.getvalue()
        # 缩小保护特有的序列是 \033[新高度;1H\n（从底部上滚）
        # 变高时不应出现此序列
        self.assertNotIn("\033[35;1H\n", output,
                          "终端变高不应触发上滚保护")

    def test_width_only_no_scroll(self):
        """仅宽度变化不触发缩小保护。"""
        self.bb._last_text = "some text"
        ...
        output = mock_out.getvalue()
        # 缩小保护特有的序列——高度不变，\033[30;1H\n 不应出现
        self.assertNotIn("\033[30;1H\n", output,
                          "仅宽度变化不应触发上滚保护")
