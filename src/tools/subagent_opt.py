...
配合 subagent 工具 background=True 模式使用。subagent 后台启动后返回
...
class SubagentOptFunc(Func):
    """按 task_id 操作后台 subagent 任务（subagent background=True 启动）。"""
    ...
    def to_tool_schema(cls):
        return {
            ...
            "function": {
                "name": "subagent_opt",
                "description": (
                    "按 task_id 操作后台 subagent 任务（由 subagent background=true 启动）。"
                ...
                "parameters": {
                    ...
                    "properties": {
                        "task_id": {
                            "type": "string",
                            "description": (
                                "后台 subagent 任务的 task_id（subagent background=True 返回的 "
                                "'sa-xxx' 格式 ID）。"
                            ),
                        },
                        "op": {
    ...
    async def execute(self) -> str:
        ...
        if not self.task_id.startswith("sa-"):
            return (f"(错误：task_id 必须是 subagent background=True 返回的 "
        ...
        if rec is None:
            return (f"(后台 subagent 任务不存在: {self.task_id}。"
                    f"请先用 subagent background=True 启动后台任务获取 task_id)")
