...
class DispatchAgents(Func):
    ...
    def __init__(self, description: str, prompt: str, target_agent_type: str = "plan_execute"):
        super().__init__()
        ...
        return {
            ...
            "function": {
                ...
                "description": (
                    ...
                    "\n- **type**：子Agent类型。plan_execute（默认，通用型，读写+bash，无路径限制）/ map（只读分析）/ review（代码审查）/ plan（计划生成，write_file/update_file 仅限 .chat/plan/ 目录）/ read_memory（只读记忆检索）/ write_memory（读写记忆更新，仅限 .chat/memory/ 目录）"
                    "\n\n"
                    "【使用限制】"
                ...
                "parameters": {
                    ...
                    "properties": {
                        ...
                        "type": {
                            "type": "string",
                            "enum": ["map", "review", "plan", "read_memory", "write_memory", "plan_execute"],
                            "description": "子Agent类型。plan_execute（默认）：排除 dispatch_agent、user_select 和 web_search，保留所有读写工具+bash，无路径限制，用于执行计划文件步骤并返回修改文件列表。map：只读分析型，仅保留 read_file/search/find/ls 等读取工具，专用于项目代码分析和地图生成。review：代码审查型，只读工具集（含 read_file/search/find/ls/web_search），专用于文件列表的 Code Review（P0-P3 分级输出）。plan：计划型，只读分析工具 + write_file/update_file（仅限写入 .chat/plan/ 目录），根据指令生成计划。read_memory：只读记忆型，仅保留 read_file/search/find/ls，专用于读取 .chat/memory/ 目录下的记忆文件。write_memory：读写记忆型，保留读工具 + write_file/update_file/mk（写入仅限 .chat/memory/ 目录），专用于维护记忆文件。",
                        },
                    },
    ...
    def from_args(cls, args: dict):
        return cls(
            ...
            target_agent_type=args.get("type", "plan_execute"),
        )
