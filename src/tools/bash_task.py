    ...
    def to_tool_schema(cls):
        return {
            ...
            "function": {
                "name": "bash_task",
                "description": (
                    "根据 task_id 操作后台 bash 任务（由 bash background=True 启动，返回 "
                    "'{\"task_id\": \"bg-xxx\", \"status\": \"running\", ...}' JSON）。"
                    "\n\n"
                    "操作（op 参数）："
                    "\n- wait：等待后台命令执行完成，返回 JSON 含 task_id / command / status / output"
                    "（output 即命令完整输出）。timeout 参数控制最长等待秒数（默认 300；传 0 无限等待）。"
                    "等待成功后该任务从后台任务列表移除（输出已由本工具拿到，不会重复插入对话）；"
                    "超时后任务继续运行且仍由 bash_task 管理，其结果不会自动作为用户消息插入对话，"
                    "可再次 wait 或 kill。"
                    "\n- kill：杀死后台命令的所有进程树（killpg 杀进程组 + /proc 递归补杀后代），"
                    "并取消该后台任务。适用于终止长时/卡死/交互式命令。"
                    "\n- stdin：向后台命令的 stdin 发送文本输入。text 参数必填；"
                    "newline 参数控制是否在文本末尾追加换行（默认 true）。"
                    "\n- keys：向后台命令发送光标/键盘消息（跨平台 ANSI/VT100 转义序列，"
                    "在 Linux/macOS/Android/Windows ConPTY 统一生效）。key 参数指定按键名。"
                    "\n\n"
                    "按键名（key 参数）支持："
                    "\n- 光标键：up / down / left / right"
                    "\n- 编辑键：home / end / page_up / page_down / insert / delete /"
                    "backspace / tab / enter / escape / space"
                    "\n- 功能键：f1 ~ f12"
                    "\n- 控制组合：ctrl_a ~ ctrl_z（含 ctrl_c 中断、ctrl_d EOF、ctrl_z 挂起、"
                    "ctrl_l 清屏等）"
                    "\n\n"
                    "典型用法："
                    "\n1. bash background=True 启动后台任务 → 拿到 task_id"
                    "\n2. bash_task op=wait task_id=bg-xxx → 等待完成并获取输出"
                    "\n3. 交互式命令：bash_task op=stdin task_id=bg-xxx text=\"输入内容\""
                    "\n4. 交互式命令：bash_task op=keys task_id=bg-xxx key=\"up\""
                    "\n5. 终止任务：bash_task op=kill task_id=bg-xxx"
                    "\n\n"
                    "【注意】"
                    "\n- task_id 必须是当前对话中 bash background=True 返回的 task_id；"
                    "任务不存在时返回明确错误。"
                    "\n- stdin/keys 发送的是原始字节，不经过 shell 解释；"
                    "子进程已退出时写入会返回错误提示。"
                    "\n- 被 bash_task 操作过的任务由本工具管理：其结果不会自动作为用户消息插入对话"
                    "（如需输出请用 op=wait 主动获取）。"
                    "\n- wait 等待期间任务仍在运行，不阻塞其他后台任务；"
                    "超时后任务继续运行，可再次 wait 或 kill。"
                ),
                "parameters": {
                    "type": "object",
                    "properties": {
