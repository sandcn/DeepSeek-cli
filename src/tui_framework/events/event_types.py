...
@dataclass(frozen=True)
class KeyPressEvent(DisplayEvent):
    """键盘按键事件。

    Attributes:
        key: 按键标识。可打印字符为原字符（如 "a", "?"），
             功能键为标准化名称（"up"/"down"/"left"/"right",
             "f1"-"f12", "enter", "escape", "backspace",
             "tab", "space", "delete", "home", "end",
             "page_up", "page_down", "insert"）。
        ctrl: Ctrl 修饰键是否按下。
        alt: Alt 修饰键是否按下。
        shift: Shift 修饰键是否按下。
        raw: 原始 ANSI 转义序列（用于调试）。
    """
    key: str = ""
    ctrl: bool = False
    alt: bool = False
...
@dataclass(frozen=True)
class MouseEvent(DisplayEvent):
    """鼠标事件（SGR 扩展模式）。

    Attributes:
        x: 鼠标列坐标（1-based）。
        y: 鼠标行坐标（1-based）。
        button: 按钮标识（"left"/"middle"/"right"/"wheel_up"/"wheel_down"）。
        action: 动作类型（"click"/"dblclick"/"drag"/"release"）。
        ctrl: Ctrl 修饰键是否按下。
        alt: Alt 修饰键是否按下。
        shift: Shift 修饰键是否按下。
    """
...
@dataclass(frozen=True)
class ResizeEvent(DisplayEvent):
    """终端尺寸变更事件。
...
@dataclass(frozen=True)
class FocusEvent(DisplayEvent):
    """焦点变更事件。

    Attributes:
        widget_id: 获得/失去焦点的控件 ID。
        gained: True=获得焦点，False=失去焦点。
    """
    widget_id: str = ""
    gained: bool = True
...
#: 所有终端输入事件的联合类型。
InputEvent = Union[KeyPressEvent, MouseEvent, ResizeEvent, FocusEvent]
