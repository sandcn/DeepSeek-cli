...
def build_fade_in_ansi(fade_frame: int, total_frames: int = 3,
    ...
    """
    if narrow or fade_frame >= total_frames:
        return ""
...
def build_bounce_ansi(frame: int, total_frames: int = 6,
                      narrow: bool = False) -> str:
...
def build_glow_ansi(frame: int, base_color: int = 45, period: int = 12) -> str:
    ...
    .. deprecated::
        ...
        统一入口为 ``tui_framework.core.effects.build_glow_ansi``，
    ...
    import warnings
    warnings.warn(
        "text_utils.build_glow_ansi is deprecated, "
        "use effects.build_glow_ansi instead. "
