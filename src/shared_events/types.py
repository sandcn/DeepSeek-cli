"""共享事件类型 — ui/chat_ui/webui 三层平等引用。
...
# ── 渲染命令共享类型 ────────────────────────────────────
# CmdSubagentSlotUpdate 原定义于 src.chat_ui.commands.types，
# 迁移至此以打破 ui ↔ chat_ui 循环依赖（P3-9）。
# ui/parallel/display.py 和 chat_ui/commands/types.py（re-export）平等引用。
