...
将原 DisplayPort 的 22 个抽象方法按职责拆分为 4 个子 Protocol：
- ToolDisplayPort   — 工具调用（6 方法）
- AgentStatusPort   — Agent 生命周期（6 方法）
- LiveMetricPort    — 实时指标（4 方法）
- SubDisplayPort    — 子显示管理（6 方法）

原 DisplayPort 保留为多重继承组合，确保 isinstance 兼容。
"""
...
class ToolDisplayPort(ABC):
    """工具调用显示子协议 — 工具从解析到执行完成的全链路展示。

    涵盖工具解析、启动、完成、批量调用及输出捕获。
    """
...
class AgentStatusPort(ABC):
    """代理状态显示子协议 — Agent 生命周期管理。

    涵盖 Agent 添加、状态更新、阶段切换、结果设置与移除。
    """
...
class LiveMetricPort(ABC):
    """实时指标显示子协议 — Token 用量与速率。

    涵盖使用量更新、速率更新、实时输入/输出 token 计数。
    """
...
class SubDisplayPort(ABC):
    """子显示管理子协议 — 并行显示与生命周期。

    涵盖子 DisplayPort 创建、上下文注入、启停及解析信息展示。
    """
...
class DisplayPort(ToolDisplayPort, AgentStatusPort, LiveMetricPort, SubDisplayPort, ABC):
    """组合显示端口 — 聚合所有子协议。

    保留此组合协议以确保向后兼容：isinstance(obj, DisplayPort) 不失效。
    新增实现应优先使用子协议以缩小依赖范围。
    """
    pass
