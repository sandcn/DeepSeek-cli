...
3. publish_summary() — 批量发布 AgentResultEvent（通过 EventPort）
"""
...
class SubAgentSpawner:
    ...
    def __init__(self, parent_agent, agent_factory, is_web: bool = False,
                 event_port=None, chat_ui_port=None):
        self.parent = parent_agent
        self._agent_factory = agent_factory
        self._is_web = is_web
        self._event_port = event_port
        self._chat_ui_port = chat_ui_port
    ...
    def _render_subagent_display(self, specs: List[Dict[str, Any]]) -> None:
        ...
        1. ChatUIConsumer 激活 → StringIO 捕获 ANSI → port.write_line()
        ...
        if port.is_active():
            # ChatUI 激活 → StringIO 捕获 ANSI → port.write_line() 统一上屏
    ...
    def _publish_tool_summary(self, results: List[Dict[str, Any]]) -> None:
        ...
        通过 EventPort.publish("agent_result", ...) 替代直接导入 ui.events。"""
        if self._event_port is None:
            return
        for r in results:
            self._event_port.publish("agent_result", {
