"""
差异渲染模块 — 从 src/ui/diff_renderer.py 迁移

职责：文件差异的终端渲染，包括行内高亮、语法高亮、上下文折叠。

迁移说明（2026-07-15）：
  - render_diff_to_ansi() 和 show_file_diff() 从 src/ui/diff_renderer.py 迁移至此
  - src/ui/diff_renderer.py 保留为向后兼容存根，从本模块重新导出
  - 外部调用点已指向 src/tui/consumer/diff_renderer
"""
