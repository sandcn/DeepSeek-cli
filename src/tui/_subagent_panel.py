...
class SubAgentPanelController:
        ...
        with self._state_lock:
            ...
            for item in records:
                ...
                slot = _AgentSlot(
                    ...
                    #   subagent tool_call_id（运行期 SubAgent 经
                    #   _record_to_parent 持久化）——主轨迹消息源恢复后凭此
                    #   把历史 subagent 合并到 subagent 工具记录（不分
    ...
    def _on_agent_added(self, event) -> None:
        self._store.add_agent(
            ...
            #   AgentAddedEvent.dispatch_label（subagent 的 tool_call_id）
            #   存入槽位——主轨迹台账按此把 subagent 记录合并到对应的
            #   subagent 工具调用记录（不分两条）；旧事件无该字段时
