...
线程安全：所有公开方法内部使用 threading.Lock 保护。
"""
...
from ...api.escape_monitor._history import (
    _read_history_file,
...
class InputBuffer:
    ...
    def __init__(self, history_file: Path, history_max_entries: int = 1000):
        self._buffer: str = ""
        self._cursor_pos: int = 0          # 当前输入光标位置（插入点索引）
        self._submitted_text: str = ""       # Enter 提交的文本，供 get_queued_input 读取
        self._input_ready = threading.Event()
        self._lock = threading.Lock()
        self._echo_callback = None  # Callable[[str, int], None] | None
        # ── 历史导航（上下箭头浏览输入历史） ──
    ...
    def handle_char(self, ch: str) -> None:
        ...
        再插入字符。
        非可打印字符（控制字符等）静默忽略（捕获逻辑由 EscapeMonitor 层负责）。
        """
        # 过滤控制字符（保留可打印字符和常见空白）
