"""ANSI 工具函数 — 从 src/ui/ansi.py 提取，消除 src/tui/ → src/ui/ 依赖

提供 ANSI 转义序列清洗、视觉宽度计算、ANSI 安全截断等工具函数。
所有 ANSI 颜色常量由 ``src.core.constants`` 统一提供，本模块不定义颜色常量。
"""
